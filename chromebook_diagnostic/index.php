...
if (isset($is_submit) && $is_submit == "Y") {
    ...
    if (decrypt($csrf_token) != $_SESSION["csrf_session"]) {
        echo redirect_to_page("index.php");
        exit();
    }
    if (empty($error)) {
        ...
        if ($count > 0) {
            ...
            $full_name   = $row_pd01[0]['first_name'] . " " . $row_pd01[0]['last_name'];
            ...
            $_SESSION['diagnose_user_full_name']    = $full_name;
        } else {
...
if (isset($is_submit2) && $is_submit2 == "Y") {
    ...
    if (empty($error)) {
        ...
        $_SESSION['product_detail'] = $product_id;
...
<body>
    <div class="container">
        ...
        if (!isset($_SESSION['is_submit'])) { ?>
            <form method="post" action="index.php?set=0">
        ...
        <?php
            ...
            <div class="row">
                <div class="col10">
                    <a href="?set=1">Reset from Start</a>
                </div>
            </div>
            <form method="post" action="index.php?set=0">
                ...
                <div class="row">
                    <div class="col10"><b>User:</b> <?php echo $_SESSION['diagnose_user_full_name']; ?> (<?php echo $_SESSION['diagnose_by_user']; ?>)</div>
                </div>
                <div class="row">
                    <div class="col2">
                        <?php
        ...
        <?php
            ...
            <div>
                ...
                // Run python script
                /*
                    $pythonScriptPath = "device_info.py";
                ...
                */
                ...
                /*
                    ...
                    $processor = trim(shell_exec("wmic cpu get Name /value"));
                    $processor = str_replace("Name=", "", $processor);

                    // Get RAM Size
                    $ramSizes = shell_exec("wmic memorychip get Capacity");
                    $ramSizes = explode("\n", trim($ramSizes));
                    array_shift($ramSizes); // Remove the header
                    $totalRam = 0;
                    foreach ($ramSizes as $size) {
                        $totalRam += intval(trim($size));
                    }
                    $totalRamGB = round($totalRam / (1024 ** 3), 2); // Convert to GB

                    // Get Storage Information
                    $storageInfo = shell_exec("wmic logicaldisk get size,freespace,caption");
                    $storageInfoLines = explode("\n", trim($storageInfo));
                    array_shift($storageInfoLines); // Remove the header
                    $storageDetails = [];
                    foreach ($storageInfoLines as $line) {
                        $parts = preg_split('/\s+/', trim($line));
                        if (count($parts) === 3) {
                            $storageDetails[] = [
                                'drive' => $parts[0],
                                'free_space' => round($parts[1] / (1024 ** 3), 2) . ' GB',
                                'total_space' => round($parts[2] / (1024 ** 3), 2) . ' GB',
                            ];
                        }
                    }

                    // Get Serial Number
                    $serialNumber = trim(shell_exec("wmic bios get serialnumber /value"));
                    $serialNumber = str_replace("SerialNumber=", "", $serialNumber);

                    // Get Model Information
                    $model = trim(shell_exec("wmic computersystem get model /value"));
                    $model = str_replace("Model=", "", $model);
                    // Display the results
                */
                // Print Hardware Infor
                /*
                    echo "<h2>Hardware Information</h2>";
                    echo "<strong>Processor:</strong> $processor<br>";
                    echo "<strong>RAM Size:</strong> $totalRamGB GB<br>";
                    echo "<strong>Serial Number:</strong> $serialNumber<br>";
                    echo "<strong>Model:</strong> $model<br>";
                    echo "<h3>Storage Details:</h3>";
                    foreach ($storageDetails as $storage) {
                        echo "Drive: " . $storage['drive'] . " - Free Space: " . $storage['free_space'] . " - Total Space: " . $storage['total_space'] . "<br>";
                    }
               */
                // Generate Battery Report and Process it calcualte health 
                /*
                    $total_storage_array = array();
                    $total_storage = "";
                    foreach ($storageDetails as $storage) {
                        $total_storage_array[] = "Drive: " . $storage['drive'] . " - Space: " . $storage['total_space'];
                    }
                    if (sizeof($total_storage_array) > 0) {
                        $total_storage = implode(", ", $total_storage_array);
                    }
                    $file_path = 'C:\\xampp\htdocs\\albert_warehouse_erp\\chromebook_diagnostic\\battries_reports\\' . $serialNumber . ".html";
                    $command = "powercfg /batteryreport /output \"$file_path\"";
                    $output = shell_exec($command);

                    if (file_exists($file_path)) {
                        echo "Battery report generated successfully";
                        // Read the battery report HTML
                        if (file_exists($file_path) && is_readable($file_path)) {
                            $html = file_get_contents($file_path);

                            preg_match('/DESIGN CAPACITY.*?([\d,]+).*?FULL CHARGE CAPACITY.*?([\d,]+)/s', $html, $matches);

                            // Extracted values
                            $design_capacity = isset($matches[1]) ? str_replace(',', '', $matches[1]) : 0;
                            $full_charge_capacity = isset($matches[2]) ? str_replace(',', '', $matches[2]) : 0;

                            // echo "<br>";
                            // echo $design_capacity;
                            // echo "<br>";
                            // echo $full_charge_capacity;
                            // echo "<br>";

                            // Calculate Battery Health
                            function calculateBatteryHealth($design_capacity, $full_charge_capacity)
                            {
                            ...
                            $battery_health = calculateBatteryHealth($design_capacity, $full_charge_capacity);

                            // Display Result
                            echo "Battery Health: " . round($battery_health, 2) . "%";
                        }
                    } else {
                        echo "Failed to generate the battery report.";
                    } 
                */ ?>
            </div>
            ...
            <div class="row">
                <div class="col10"><b>PO#:</b> <?php echo $_SESSION['po_no']; ?>, Product: <?php echo $_SESSION['product_detail']; ?></div>
            </div>
            <div class="row row_speaker">
                <div class="col4">
                    <!-- Speaker Test -->
                    <div id="speaker">
                        <h2>Speaker Test</h2>
                ...
                <div class="col3 speaker_btn">
                    <button id="speaker_btn" class="speaker_btn bg_yellow">Next</button>
                </div>
            </div>
            <div class="row row_mic" style="display: none;">
                <div class="col3">
                    <!-- Microphone Test -->
                    <div id="mic">
                ...
                <div class="col2 mic_btn">
                    <button id="mic_btn" class="previous_next_btn bg_yellow">Next</button>
                </div>
                <div class="col2 mic_previous_btn">
                    <button id="mic_previous_btn" class="previous_next_btn bg_red">Previous</button>
                </div>
            </div>
            <div class="row row_battery" style="display: none;">
                <div class="col3">
                    <!-- Battery Test -->
                    <div id="battery">
                        <h2>Battery Test</h2>
                ...
                <div class="col2 battery_btn">
                    <button id="battery_btn" class="previous_next_btn bg_yellow">Next</button>
                </div>
                <div class="col2 battery_previous_btn">
                    <button id="battery_previous_btn" class="previous_next_btn bg_red">Previous</button>
                </div>
            </div>
            <div class="row_camera" style="display: none;">
                <div class="row">
                    <div class="col12">
                        <!-- Camera Test -->
                        <div id="camera">
                ...
                <div class="row">
                    <div class="col2 camera_btn">
                        <button id="camera_btn" class="previous_next_btn bg_yellow">Next</button>
                        <br><br><br><br><br>
                    </div>
                    <div class="col2 camera_previous_btn">
                        <button id="camera_previous_btn" class="previous_next_btn bg_red">Previous</button>
                        <br><br><br><br><br>
                    </div>
                </div>
            </div>
            <!-- Keyboard Test -->
            <div class="row_keyboard" style="display: none;">
                <div class="row">
                    <div class="col8">
                        <div class="keyboard">
                            <h2>Keyboard Test</h2>
                    ...
                    <div class="col2 keyboard_btn">
                        <button id="keyboard_btn" class="previous_next_btn bg_yellow">Next</button>
                        <br><br><br><br><br><br><br><br><br><br>
                    </div>
                    <div class="col2 keyboard_previous_btn">
                        <button id="keyboard_previous_btn" class="previous_next_btn bg_red">Previous</button>
                        <br><br><br><br><br><br><br><br><br><br>
                    </div>
                </div>
            </div>
            <div class="row other_fields" style="display: none;">
                <br><br>
                <div class="col2">
                    <?php
                ...
                <div class="col2">
                    <?php
                    $field_name     = "battery";
                    $field_label    = "Battery Percentage %"; ?>
                    <label>
                        <?= $field_label; ?>:
                    ...
                    <input type="number" class="input_text" name="<?= $field_name; ?>" id="<?= $field_name; ?>_per" placeholder="<?= $field_label; ?>" value="<?php if (isset(${$field_name})) {
                                                                                                                                                                    echo ${$field_name};
                                                                                                                                                                } ?>" />
                </div>
                <div class="col3">
                ...
                <div class="col2">
                    ...
                    <input type="text" class="input_text" name="<?= $field_name; ?>" id="<?= $field_name; ?>_per" placeholder="<?= $field_label; ?>" value="<?php if (isset(${$field_name})) {
                                                                                                                                                                echo ${$field_name};
                                                                                                                                                            } ?>" />
                </div>
                <div class="col2">
                    <?php
                    $field_name     = "totalRamGB";
                    $field_label    = "RAM"; ?>
                    <label>
                        <?= $field_label; ?>:
                ...
                <div class="row">
                    ...
                    <div class="col2 final_previous_btn">
                        <button id="final_previous_btn" class="speaker_btn bg_red">Previous</button>
                    </div>
                    <br><br>
                </div>
            </div>
    ...
    if (isset($_SESSION['is_submit']) && isset($_SESSION['is_submit2'])) { ?>
        <script>
            ...
            navigator.getBattery().then((battery) => {
                ...
                // Add event listeners to track changes dynamically
                ...
                function updateBatteryStatus(battery) {
                    ...
                    batteryPercentage.textContent = `Battery Percentage: ${Math.round(battery.level * 100)}%`;

                    let betteryTimeSeconds = battery.chargingTime;
                    if (battery.chargingTime === Infinity) {
                        batteryChargingTime.textContent = 'Charging Time:  N/A';
                    } else {
                        // Calculate hours, minutes, and seconds
                        let hours = Math.floor(betteryTimeSeconds / 3600); // Get total hours
                        let minutes = Math.floor((betteryTimeSeconds % 3600) / 60); // Get remaining minutes
                        let remainingSeconds = Math.floor(betteryTimeSeconds % 60); // Get remaining seconds

                        // Return the formatted time
                        batteryChargingTime.textContent = `${hours} hours, ${minutes} minutes, ${remainingSeconds} seconds`;
                    }
                }
            });
            ...
            document.getElementById('submitTest').addEventListener('click', () => {
                ...
                let battery = document.getElementById('battery_per').value;
                let sub_location_id = document.getElementById('sub_location_id').value;
                let processor = document.getElementById('processor').value;
                let totalRamGB = document.getElementById('totalRamGB').value;
                let serialNumber = document.getElementById('serialNumber').value;
                let model = document.getElementById('model').value;
                let total_storage = document.getElementById('total_storage').value;
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#mic_previous_btn').on('click', function() {
                $('.row_mic').hide();
                $('.row_speaker').show();
            });
            $('#battery_btn').on('click', function() {
                $('.row_battery').hide();
                $('.row_camera').show();
            });
            $('#battery_previous_btn').on('click', function() {
                $('.row_battery').hide();
                $('.row_mic').show();
            });
            ...
            $('#camera_previous_btn').on('click', function() {
                $('.row_camera').hide();
                $('.row_battery').show();
            });
            ...
            $('#keyboard_previous_btn').on('click', function() {
                $('.row_keyboard').hide();
                $('.row_camera').show();
            });
            $('#final_previous_btn').on('click', function() {
                $('.other_fields').hide();
                $('.row_keyboard').show();
            });
