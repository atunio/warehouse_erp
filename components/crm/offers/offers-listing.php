...
if (isset($cmd) && ($cmd == 'disabled' || $cmd == 'enabled') && access("delete_perm") == 0) {
	...
	if (isset($cmd) && $cmd == 'disabled') {
		$sql_c_upd = "UPDATE offers set enabled = 0,
	...
	if (isset($cmd) && $cmd == 'enabled') {
		$sql_c_upd = "UPDATE offers set 	enabled 	= 1,
											update_date = '" . $add_date . "' ,
											update_by 	= '" . $_SESSION['username'] . "' ,
											update_ip 	= '" . $add_ip . "'
					WHERE id = '" . $id . "' ";
				...
				<div class="row">
					...
					<div class="col s2 m6 l6">
						<?php if (access("add_perm") == 1) { ?>
							<a class="btn waves-effect waves-light green darken-1 breadcrumbs-btn right" href="?string=<?php echo encrypt("module=" . $module . "&module_id=" . $module_id . "&page=add&cmd=add&cmd2=add") ?>" data-target="dropdown1">
								Add New
							</a>
						<?php } ?>
					</div>
												...
												<tbody>
													...
													if ($count_cl > 0) {
														...
														foreach ($row_cl as $data) {
															...
															<tr>
																<?php
																// detail_desc product_price product_qty vender_id product_id product_desc, c.vender_name															
																?>
