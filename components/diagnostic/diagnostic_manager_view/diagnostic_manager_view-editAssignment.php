...
if (isset($test_on_local) && $test_on_local == 1 && $cmd == 'add' || $cmd == 'edit') {
	$assignment_no	= "A-".date('Ymd-His');
}
$db 					= new mySqlDB;
...
if ($cmd == 'edit') {
	$title_heading = "Update Assignment No";
	$button_val = "Save";
}
if ($cmd == 'add') {
	$title_heading 	= "Add Assignment No";
	$button_val 	= "Add";
	$id 			= "";
}
if ($cmd == 'edit' && isset($id)) {
	// $location_id
	// $bin_user_id
	$sql_ee				= "SELECT a.* FROM users_bin_for_diagnostic a WHERE a.id = '" . $id . "' "; // echo $sql_ee;
...
if (isset($is_Submit) && $is_Submit == 'Y') {
	if (isset($assignment_no) && $assignment_no == "") {
		$error['assignment_no']	= "Required";
		$assignment_no_valid 		= "invalid";
	}
	if (isset($bin_user_id) && $bin_user_id == "") {
		$error['bin_user_id']	= "Required";
		$bin_user_id_valid 		= "invalid";
	}
	if (empty($error)) {
		 if ($cmd == 'edit') {
			if (access("edit_perm") == 0) {
				...
				$sql_dup	= " SELECT a.* FROM users_bin_for_diagnostic a
								WHERE  assignment_no	= '" . $assignment_no . "' 
								AND a.id != '" . $id . "'";
				$result_dup	= $db->query($conn, $sql_dup);
				$count_dup	= $db->counter($result_dup);
				if ($count_dup == 0) {
					$sql_c_up = "UPDATE users_bin_for_diagnostic SET 	assignment_no	= '" . $assignment_no . "',
																		bin_user_id		= '" . $bin_user_id . "', 
					...
					$error['msg'] = "This Assignment# is already exist or assigned to another user.";
				}
			}
		}
	}
					...
					<form method="post" autocomplete="off">
						...
						<div class="row">
							<div class="input-field col m3 s12">
								<?php
								$field_name 	= "assignment_no";
								$field_label 	= "Assignment No";
								?>
							...
							<div class="input-field col m4 s12">
                                 <?php
                                    ...
                                    $field_id     	= "bin_user_id1";
                                    $field_label    = "Users"; 
                                    $sql1           = " SELECT id, CONCAT(COALESCE(first_name, ''), ' ', COALESCE(last_name, '')) AS user_full_name
														FROM users 
                                 ...
                                 <div class="select2div">
                                     <select id="<?= $field_id; ?>" name="<?= $field_name; ?>" class="select2 browser-default select2-hidden-accessible validate <?php if (isset(${$field_name . "_valid"})) {
                                            ...
                                            if ($count1 > 0) {
												...
												<option value="<?php echo $data2['id']; ?>" <?php if (isset($bin_user_id) && $bin_user_id == $data2['id']) { ?> selected="selected" <?php } ?>><?php echo $data2['user_full_name']; ?></option>
                                         <?php }
                                            } ?>
                                     </select>
                                     <label for="<?= $field_id; ?>">
