...
if (isset($is_Submit) && $is_Submit == "Y") {
	...
	if (empty($error)) {
		...
		if ($date_from == "" && $date_to != "") {
			...
			$sql_cl .= "AND date_format(a.att_date, '%Y%m%d') <= '" . $date_to1 . "' ";
		} else if ($date_to == "" && $date_from != "") {
			$date_from1 = str_replace("-", "", convert_date_mysql_slash($date_from));
			$sql_cl .= "AND date_format(a.att_date, '%Y%m%d') >= '" . $date_from1 . "' ";
		} else if ($date_from != "" && $date_to != "") {
			$date_to1 	= str_replace("-", "", convert_date_mysql_slash($date_to));
			$date_from1 = str_replace("-", "", convert_date_mysql_slash($date_from));
			$sql_cl    .= "AND date_format(a.att_date, '%Y%m%d') BETWEEN '" . $date_from1 . "' AND '" . $date_to1 . "' ";
		}
		$sql_cl 	   .= " ORDER BY date_format(a.att_date, '%Y%m%d') DESC ";
...
$page_heading = "Employee Attendance";
?>
<!-- BEGIN: Page Main-->
			...
			<div class="container">
				<div class="row">
					...
					<div class="col s12 m2">
						<a class="btn waves-effect waves-light green darken-1 breadcrumbs-btn right" href="?string=<?php echo encrypt("module=" . $module . "&page=single&cmd=add") ?>" data-target="dropdown1">
							Single Employee Attendance
						</a>
					</div>
					<div class="col s12 m2">
						<a class="btn waves-effect waves-light green darken-1 breadcrumbs-btn right" href="?string=<?php echo encrypt("module=" . $module . "&page=all_emp&cmd=add") ?>" data-target="dropdown1">
							All Employees Attendance
						</a>
					</div>
					<div class="col s12 m2">
						<a class="btn waves-effect waves-light amber darken-1 breadcrumbs-btn right" href="?string=<?php echo encrypt("module=" . $module . "&page=import&cmd=add") ?>" data-target="dropdown1">
							Import Excel File
						</a>
					</div>
				</div>
														...
														if ($count_cl > 0) {
															...
															foreach ($row_cl as $data) {
																...
																<tr data-id="<?php echo $id; ?>">
																	...
																	<td><?php echo dateformat2($data['att_date']); ?></td>
																	<td><?php echo $data['att_time_in']; ?></td>
																	<td><?php echo $data['att_time_out']; ?></td>
																	<td><?php echo $data['is_absent']; ?></td>
																	<td class="text-align-center">
																		...
																		&nbsp;&nbsp;
																		<a class="" href="?string=<?php echo encrypt("module=" . $module . "&page=single&cmd=edit&id=" . $data['id']) ?>">
