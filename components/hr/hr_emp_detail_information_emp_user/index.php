...
if (isset($_SESSION["username"]) && isset($_SESSION["user_id"]) && isset($_SESSION["schoolDirectory"]) && $_SESSION["schoolDirectory"] == 'School') {
	$check_module_permission = check_module_permission_user($db, $conn, "hr_employees_information", $_SESSION["user_id"], $_SESSION["school_admin_id"], $_SESSION["user_type"], $_SESSION["db_name"]);
	if ($check_module_permission == "") {
		header("location: " . $directory_path_for_module_index . "signout");
		exit();
	} else {
		...
		if (isset($_POST['type']) && $_POST['type'] == 'update') {
			...
			$sql_c_up = "UPDATE " . $selected_db_name . ".employee_profile SET 	enabled 		= '" . $new_status . "',
																				update_date 	= '" . $add_date . "',
																				update_by 	 	= '" . $_SESSION['username'] . "',
																				update_ip 	 	= '" . $add_ip . "'
						WHERE id = '" . $record_id . "' AND school_admin_id= '" . $school_admin_id . "' ";
