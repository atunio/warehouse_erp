...
if(isset($is_Submit_experience) && $is_Submit_experience == 'Y'){ 
	...
	if(isset($e_job_title) && $e_job_title == ""){ 					$error['msg'] = "Enter Job Title"; $e_job_title_valid = "invalid";}  
	if(isset($e_job_role) && $e_job_role == ""){ 					$error['msg'] = "Enter Job Role"; $e_job_role_valid = "invalid";}  
	if(!isset($id) || (isset($id)  && ($id == "0" || $id == ""))){ $error['msg'] = "Please add employee information first"; } 
	if(isset($e_date_from) && $e_date_from == ""){ 					$error['msg'] = "Enter Appointment Date"; $e_date_from_valid = "invalid";}
	else{
		$e_date_from1 	= "0000-00-00"; 
		$e_date_from1 	= convert_date_mysql_slash($e_date_from);
	} 
	if(isset($e_date_to) && $e_date_to == ""){ 						$error['msg'] = "Enter Termination Date"; $e_date_to_valid = "invalid";}
	else{
		$e_date_to1 	= "0000-00-00"; 
		$e_date_to1 	= convert_date_mysql_slash($e_date_to);
	}
	if(isset($e_company) && $e_company == ""){ 						$error['msg'] = "Enter Company Name"; $e_company_valid = "invalid";}
	if(isset($e_job_description) && $e_job_description == ""){ 		$error['msg'] = "Enter Jb Description"; $e_job_description_valid = "invalid";}  
	if(empty($error)){
		...
		if($cmd3 == 'add'){ 
			...
			if($counter_ee1 == 0){
				...
				if($ok){
					$e_job_title = $e_job_role = $e_job_title = $e_company = $e_job_description = $e_date_from = $e_date_to = "";
