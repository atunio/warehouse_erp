...
if (isset($is_Submit_Education) && $is_Submit_Education == 'Y') {
	...
	if (empty($error)) {
		...
		if ($cmd2 == 'add') {
			...
			if ($counter_ee1 == 0) {
				$sql = "INSERT INTO " . $selected_db_name . ".employee_education(subscriber_users_id, emp_profile_id, e_institution_name, date_from, date_to, degree_name, study_area, add_date, add_by, add_ip)
						VALUES('" . $subscriber_users_id . "', '" . $id . "', '" . $e_school . "', '" . $date_from1 . "', '" . $date_to1 . "', '" . $degree_name . "', '" . $study_area . "', '" . $add_date . "', '" . $_SESSION['username'] . "', '" . $add_ip . "')";

				$ok = $db->query($conn, $sql);
				if ($ok) {
					$e_school = $date_from = $date_to = $degree_name = $study_area = "";
			...
			check_id($db, $conn, $detail_id, "employee_education", $subscriber_users_id, $selected_db_name);
			$sql_ee1 			= "	SELECT a.* FROM " . $selected_db_name . ".employee_education a 
									WHERE a.subscriber_users_id 	= '" . $subscriber_users_id . "'   
									AND a.emp_profile_id		= '" . $id . "'    
									AND a.degree_name			= '" . $degree_name . "' 
			...
			$counter_ee1			= $db->counter($result_ee1);
			if ($counter_ee1 == 0) {
				$sql_c_up = "UPDATE " . $selected_db_name . ".employee_education SET 	e_institution_name			= '" . $e_school . "',
