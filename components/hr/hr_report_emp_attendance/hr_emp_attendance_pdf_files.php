...
$report_data = '<table class="top" cellpadding="5">
				...
				<h2>Attendace Summary Report</h2>';
				...
				<table class="first" cellpadding="5">
					<thead>
						<tr> 
							...
							<th width="10%">Presents</th>
							<th width="10%">Absents</th>
					...
					if($count_cl >0){
						...
						foreach($row_cl as $data_detail){
							...
							$sql_att 	= "	SELECT a.is_absent, COUNT(*) AS total_days
											FROM ".$selected_db_name.".emp_attendance a
											WHERE a.school_admin_id 	= '".$school_admin_id."'
											AND a.emp_id				='".$emp_id."' 
											AND DATE_FORMAT(a.att_date, '%Y%m%d') BETWEEN '".$date_from2."' AND '".$date_to2."'
											AND a.enabled 				= 1
											GROUP BY a.is_absent
							...
							if($count_att >0 ){
								...
								$report_data .=" <tr nobr='true'> 
													...
													<td align='center'><b>".$presents." (".round((($presents/$total_days)*100))."%)</b></td>
													<td align='center'><b>".$absents." (".round((($absents/$total_days)*100))."%)</b></td>
													<td align='center'><b>".$leaves." (".round((($leaves/$total_days)*100))."%)</b></td>
													<td align='center'><b>".$total_days."</b></td> 
												</tr>";
							} 
							$i++;
						}
