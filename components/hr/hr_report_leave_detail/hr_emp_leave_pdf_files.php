...
$sql_cl 	= "	SELECT  a.*, c.department_name, d.designation, e.leave_type, e.leave_category, e.leave_from, e.leave_to, e.days_deduction
				...
				INNER JOIN ".$selected_db_name.".emp_leave e ON e.emp_id = a.id 
...
$sql_cl 	.= " AND DATE_FORMAT(e.leave_from, '%Y%m%d') >= '".$date_from2."' 
				AND DATE_FORMAT(e.leave_to, '%Y%m%d') <= '".$date_to2."' ";
$sql_cl 	.= " ORDER BY e.id DESC ";  
//echo $sql_cl;die;
				...
				<table class="first" cellpadding="5">
					<thead>
						<tr> 
							...
							<th width="13%">Category / Type</th>
							<th width="20%">Date</th>
							<th width="10%">TotalDays</th>
						</tr>
					</thead>
					<tbody>';
					$i = 1;
					if($count_cl >0){
						...
						foreach($row_cl as $data_detail){
							...
							$leave_type 				= $data_detail['leave_type'];
							$leave_category 			= $data_detail['leave_category'];
							$leave_from 				= dateformat2($data_detail['leave_from']);
							$leave_to 					= dateformat2($data_detail['leave_to']);
							$days_deduction 			= $data_detail['days_deduction']; 
							if($leave_category == "Half Day" || $leave_category == "Full Day"){
								$leave_from = "";
								$leave_to = "";
							}
							$report_data .=" <tr nobr='true'> 
												...
												<td align='center'><b>".$leave_category."</b> <br> <b>".$leave_type."</b></td> 
												<td align='center'><b>From ".$leave_from." <br> To ".$leave_to."</b></td>
												<td align='center'><b>".$days_deduction."</b></td>
											</tr>"; 
							$i++;
						}
					}
...
$mpdf->SetTitle('Leaves Report '.$subject_name);
$file_name = "Leaves Report ".$subject_name.".pdf";
