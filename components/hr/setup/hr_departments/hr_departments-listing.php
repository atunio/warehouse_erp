...
if (isset($cmd) && ($cmd == 'disabled' || $cmd == 'enabled') && access("delete_perm") == 0) {
	...
	if (isset($cmd) && $cmd == 'disabled') {
		$sql_c_upd = "UPDATE departments set enabled = 0,
	...
	if (isset($cmd) && $cmd == 'enabled') {
		$sql_c_upd = "UPDATE departments set enabled = 1,
											update_date = '" . $add_date . "' ,
											update_by 	= '" . $_SESSION['username'] . "' ,
											update_ip 	= '" . $add_ip . "'
					WHERE id = '" . $id . "' ";
...
$sql_cl 		= "	SELECT a.*
					...
					WHERE a.subscriber_users_id = '" . $subscriber_users_id . "'
					 "; //echo $sql_cl;
if (isset($flt_department) && $flt_department != "") {
	$sql_cl 	.= " AND a.id = '" . trim($flt_department) . "' ";
}
if (isset($is_enabled_disabled) && $is_enabled_disabled != "") {
	$sql_cl			.= " AND a.enabled = '" . $is_enabled_disabled . "' ";
}
$sql_cl	.= " ORDER BY a.sort_order "; // echo $sql_cl;
								...
								<form method="post" autocomplete="off" enctype="multipart/form-data">
									...
									<div class="row">
										<div class="input-field col m2 s12">
											<?php
											$field_name 	= "flt_department";
											$field_label 	= "Department";
											$sql1 			= "SELECT a.* FROM departments a  ORDER BY department_name ";
