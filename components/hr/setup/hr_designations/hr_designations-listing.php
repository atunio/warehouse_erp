...
$sql_cl = "	SELECT a.*
			...
			";
if (isset($flt_designation) && $flt_designation != "") {
	$sql_cl 	.= " AND a.id = '" . trim($flt_designation) . "' ";
}
if (isset($is_enabled_disabled) && $is_enabled_disabled != "") {
	$sql_cl			.= " AND a.enabled = '" . $is_enabled_disabled . "' ";
}
$sql_cl	.= " ORDER BY a.enabled DESC, a.designation  "; // echo $sql_cl;
								...
								<form method="post" autocomplete="off" enctype="multipart/form-data">
									...
									<div class="row">
										<div class="input-field col m2 s12">
											<?php
											$field_name 	= "flt_designation";
											$field_label 	= "Designation";
											$sql1 			= "SELECT a.* FROM designations a WHERE a.subscriber_users_id = '" . $subscriber_users_id . "'  ORDER BY designation ";
												...
												<select id="<?= $field_name; ?>" name="<?= $field_name; ?>" class=" select2 browser-default select2-hidden-accessible validate <?php if (isset(${$field_name . "_valid"})) {
													...
													if ($count1 > 0) {
														...
														foreach ($row1 as $data2) { ?>
															<option value="<?php echo $data2['id']; ?>" <?php if (isset(${$field_name}) && ${$field_name} == $data2['id']) { ?> selected="selected" <?php } ?>><?php echo $data2['designation']; ?></option>
													<?php }
													} ?>
												</select>
