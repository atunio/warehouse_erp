...
$supported_column_titles 	= array("package_name", "stock_in_hand", "avg_price", "category", "category_type", "compatible_product_ids");
$duplication_columns 		= array("package_name",);
$required_columns 			= array("package_name", "category", "category_type", "compatible_product_ids");
							...
							if ($key != 'is_insert') {
								if ($key == 'product_category') {
									...
									if ($data != '' && $data != NULL && $data != '-' && $data != 'blank') {
										$sql1 			= "SELECT * FROM " . $table . " WHERE category_name = '" . $data . "' ";
										$result1 		= $db->query($conn, $sql1);
										$count1 		= $db->counter($result1);
										if ($count1 > 0) {
								...
								} else if ($key == 'compatible_product_ids') {
									if ($data != '' && $data != NULL && $data != '-' && $data != 'blank') {
										...
										foreach ($product_id_array as $array_element) {
											...
											if ($count1 > 0) {
												$row1 					= $db->fetch($result1);
												$all_product_ids_array[] 	= $row1[0]['id'];
											}
										}
							...
							<div class="col m12 s12">
								...
								<table class="bordered striped" style="padding: 0px; width: 80%;">
									...
									foreach ($supported_column_titles as $s_heading) {
										...
										if ($s_heading == 'category_type') {
											$cell_format = "Text (Package Material / Parts) Only";
										}
										if ($s_heading == 'stock_in_hand') {
											$cell_format = "Number";
										}
										if ($s_heading == 'case_pack') {
											$cell_format = "Number";
										}
										...
										<tbody>
											...
											foreach ($data as $row_c1) {
												...
												foreach ($row_c1 as $cell_c1) {
													...
													if ($column_name_c1 == 'category') {
														$column_name_c1 = "product_category";
													}
													if ($column_name_c1 == 'sku') {
														$column_name_c1 = "product_sku";
													}
