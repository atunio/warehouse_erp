...
$title_heading 			= "Import Packages Materials in Products Madatory/Optional";
$button_val 			= "Preview";
extract($_POST);
...
$supported_column_titles 	= array("product_id", "sku_code", "is_mandatory");
$duplication_columns 		= array("sku_code");
$required_columns 			= array("product_id", "sku_code", "is_mandatory");
	...
	if (empty($error)) {
		...
		if (isset($all_data) && sizeof($all_data) > 0) {
			foreach ($duplication_columns  as $dup_data) {
				...
				$duplicate_colum_values2 	= array_unique(array_column($all_data, 'product_id'));
				$duplicate_colum_values3 	= array_unique(array_column($all_data, 'is_mandatory'));
				// sku_code is_mandatory product_id
				$c = 0;
				foreach ($duplicate_colum_values  as $key => $duplicate_colum_values1) {
					$db_column = $dup_data;
					$dup_product_id 	= $duplicate_colum_values2[$c];
					$dup_is_mandatory 	= $duplicate_colum_values3[$c];

					if (($duplicate_colum_values1 == '' || $duplicate_colum_values1 == NULL || $duplicate_colum_values1 == '-' || $duplicate_colum_values1 == 'blank')
						|| ($dup_product_id == '' || $dup_product_id == NULL || $dup_product_id == '-' || $dup_product_id == 'blank')
						|| ($dup_is_mandatory == '' || $dup_is_mandatory == NULL || $dup_is_mandatory == '-' || $dup_is_mandatory == 'blank')
					) {
						$duplicate_data_array[$key][$db_column][] = $duplicate_colum_values1;
						if (!isset($error['msg'])) {
							$error['msg'] = "Incomplete " . $dup_data . ": <span class='color-blue'>" . $duplicate_colum_values1 . "</span>.";
						} else {
							...
							if ($key != 'is_insert') {
								if ($key == 'product_id') {
									if ($data != '' && $data != NULL && $data != '-' && $data != 'blank') {
										$sql1 			= "SELECT * FROM products WHERE product_uniqueid = '" . $data . "' ";
										$result1 		= $db->query($conn, $sql1);
										$count1 		= $db->counter($result1);
										if ($count1 > 0) {
									...
									if ($data != '' && $data != NULL && $data != '-' && $data != 'blank') {
										$sql1 			= "SELECT * FROM packages WHERE sku_code = '" . $data . "' ";
										$result1 		= $db->query($conn, $sql1);
										$count1 		= $db->counter($result1);
										if ($count1 > 0) {
											$row1 = $db->fetch($result1);
										...
										<tbody>
											...
											foreach ($data as $row_c1) {
												...
												foreach ($row_c1 as $cell_c1) {
													...
													if ($column_name_c1 == 'sku_code') {
														$db_name_c1 = 'sku_code';
														$sql_dup	= " SELECT * FROM packages WHERE " . $db_name_c1 . "	= '" . htmlspecialchars($cell_c1) . "' ";
													...
													if ($column_name_c1 == 'product_id') {
														$sql_dup	= " SELECT * FROM products WHERE product_uniqueid	= '" . htmlspecialchars($cell_c1) . "' ";
														$result_dup	= $db->query($conn, $sql_dup);
														$count_dup	= $db->counter($result_dup);
														if ($count_dup == 0) {
															${$column_name_c1 . "_array"}[] = $cell_c1;
