...
$supported_column_titles 	= array("status_name");
$duplication_columns 		= array("status_name");
$required_columns 			= array("status_name");
	...
	if (empty($error)) {
		...
		if (isset($all_data) && sizeof($all_data) > 0) {
			...
			foreach ($all_data  as $data1) { 
				if (isset($data1['status_name']) && $data1['status_name'] != '' && $data1['status_name'] != NULL && $data1['status_name'] != 'blank') {
					$status_name2 	= $data1['status_name'];
					...
					if(isset($ids_already) && isset($status_name2) && in_array($status_name2, $ids_already)){
						if ($update_column != "" ) { // ONLY IF one field 
							$sql6 = "UPDATE " . $selected_db_name . "." . $master_table . " SET update_date 			= '" . $add_date . "', 
																								...
																								".$update_column."
									WHERE status_name 	= '".$status_name2."' "; //echo "<br><br>".$sql6;
