	...
	if (isset($is_Submit_del) && $is_Submit_del == 'Y') {
		...
		if (empty($error)) {
			...
			foreach ($record_ids as $data_d) {
				if ($is_enabled != "") {
					$sql_dl = "UPDATE inventory_status SET enabled = '" . $is_enabled . "' WHERE id = '" . $data_d . "' ";
...
$sql_cl 		= "	SELECT  a.* FROM inventory_status a  WHERE 1=1 ";
if (isset($flt_status) && $flt_status != "") {
	$sql_cl 	.= " AND a.id = '" . trim($flt_status) . "' ";
}
									...
									<div class="row">
										<div class="input-field col m3 s12">
											...
											<div class="select2div">
												<?php
												$field_name     = "flt_status";
												$field_label    = "Inventory Status";

												$sql1           = " SELECT  a.* FROM inventory_status a ORDER BY a.status_name";
													...
													if ($count_cl > 0) {
														...
														foreach ($row_cl as $data) {
															...
															<tr>
																...
																<td class="col-<?= set_table_headings($table_columns[$col_no]); ?>">
																	<?php echo ucwords(strtolower($data['status_type']));
																	$col_no++; ?>
																</td>
