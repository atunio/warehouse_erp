...
if (isset($test_on_local) && $test_on_local == 1 && $cmd == 'add') {
 	$processor_name	= "PR-".date('Ymd-His');
}
$db 					= new mySqlDB;
...
if ($cmd == 'edit') {
	$title_heading = "Update Processor";
	$button_val = "Save";
}
if ($cmd == 'add') {
	$title_heading 	= "Add Processor";
	$button_val 	= "Add";
	$id 			= "";
}
if ($cmd == 'edit' && isset($id)) {

	$sql_ee				= "SELECT a.* FROM processors a WHERE a.id = '" . $id . "' "; // echo $sql_ee;
	$result_ee			= $db->query($conn, $sql_ee);
	$row_ee				= $db->fetch($result_ee);
	$processor_name		= $row_ee[0]['processor_name'];
}
extract($_POST);
...
if (isset($is_Submit) && $is_Submit == 'Y') {
	if (isset($processor_name) && $processor_name == "") {
		$error['processor_name']	= "Required";
	...
	if (empty($error)) {
		if ($cmd == 'add') {
			if (access("add_perm") == 0) {
				...
				$sql_dup	= " SELECT a.* FROM processors a  WHERE a.processor_name	= '" . $processor_name . "' ";
				$result_dup	= $db->query($conn, $sql_dup);
				$count_dup	= $db->counter($result_dup);
				if ($count_dup == 0) {
					$sql6 = "INSERT INTO " . $selected_db_name . ".processors(subscriber_users_id, processor_name, add_date, add_by, add_ip)
							VALUES('" . $subscriber_users_id . "', '" . $processor_name . "', '" . $add_date . "', '" . $_SESSION['username'] . "', '" . $add_ip . "')";
					...
					$sql_c_up = "UPDATE processors 	SET enabled			= '1', 
			...
			if (access("edit_perm") == 0) {
				...
				$sql_dup	= " SELECT a.* FROM processors a WHERE a.processor_name = '" . $processor_name . "' AND a.id != '" . $id . "'";
				$result_dup	= $db->query($conn, $sql_dup);
				$count_dup	= $db->counter($result_dup);
				if ($count_dup == 0) {
					$sql_c_up = "UPDATE processors SET processor_name	= '" . $processor_name . "', 
															 update_date	= '" . $add_date . "',
															 update_by		= '" . $_SESSION['username'] . "',
															 update_ip		= '" . $add_ip . "'
								WHERE id = '" . $id . "' ";
					...
					$ok = $db->query($conn, $sql_c_up);
					if ($ok) {
						$sql_c_up = "UPDATE processors 	SET enabled			= '0', 
															update_date		= '" . $add_date . "',
															update_by		= '" . $_SESSION['username'] . "',
															update_ip		= '" . $add_ip . "'
									WHERE id = '" . $id . "' ";
