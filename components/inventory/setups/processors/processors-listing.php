	...
	if (isset($is_Submit_del) && $is_Submit_del == 'Y') {
		...
		if (empty($error)) {
			...
			foreach ($record_ids as $data_d) {
				if ($is_enabled != "") {
					$sql_dl = "UPDATE processors SET enabled = '" . $is_enabled . "' WHERE id = '" . $data_d . "' ";
...
if (isset($cmd) && ($cmd == 'disabled' || $cmd == 'enabled') && access("delete_perm") == 0) {
	...
	if (isset($cmd) && $cmd == 'disabled') {
		$sql_c_upd = "UPDATE processors set enabled = 0,
	...
	if (isset($cmd) && $cmd == 'enabled') {
		$sql_c_upd = "UPDATE processors set enabled = 1,
											update_date = '" . $add_date . "' ,
											update_by 	= '" . $_SESSION['username'] . "' ,
											update_ip 	= '" . $add_ip . "'
					WHERE id = '" . $id . "' ";
...
$sql_cl 		= "	SELECT  a.* FROM processors a WHERE 1=1  ";
if (isset($flt_processor) && $flt_processor != "") {
	$sql_cl 	.= " AND a.id = '" . trim($flt_processor) . "' ";
}
...
$page_heading 	= "List of Processors";
?>
<!-- BEGIN: Page Main-->
									...
									<div class="row">
										<div class="input-field col m3 s12">
											...
											<div class="select2div">
												<?php
												$field_name     = "flt_processor";
												$field_label    = "Processor";

												$sql1           = " SELECT  a.* FROM processors a ORDER BY processor_name";
												...
												<select id="<?= $field_name; ?>" name="<?= $field_name; ?>" class=" select2 browser-default select2-hidden-accessible validate <?php if (isset(${$field_name . "_valid"})) {
													...
													if ($count1 > 0) {
														...
														foreach ($row1 as $data2) { ?>
															<option value="<?php echo $data2['id']; ?>" <?php if (isset(${$field_name}) && ${$field_name} == $data2['id']) { ?> selected="selected" <?php } ?>><?php echo $data2['processor_name']; ?></option>
													<?php }
													} ?>
												</select>
								...
								<div class="row">
									...
									<div class="col m10 s12">
										<div class="text_align_right">
											<?php
											$table_columns	= array('SNo', 'Processor', 'Actions');
											$k 				= 0;
														...
														foreach ($row_cl as $data) {
															...
															<tr>
																...
																<td class="col-<?= set_table_headings($table_columns[$column_no]); ?>">
																	...
																	<?php echo ucwords(strtolower($data['processor_name']));
																	$column_no++; ?>
																</td>
