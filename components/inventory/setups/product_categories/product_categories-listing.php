...
if (isset($cmd) && ($cmd == 'disabled' || $cmd == 'enabled') && access("delete_perm") == 0) {
	...
	if (isset($cmd) && $cmd == 'disabled') {
		$sql_c_upd = "UPDATE product_categories set enabled = 0,
	...
	if (isset($cmd) && $cmd == 'enabled') {
		$sql_c_upd = "UPDATE product_categories set enabled = 1,
											update_date = '" . $add_date . "' ,
											update_by 	= '" . $_SESSION['username'] . "' ,
											update_ip 	= '" . $add_ip . "'
					WHERE id = '" . $id . "' ";
...
$sql_cl 		= "	SELECT  a.* FROM product_categories a WHERE 1=1"; 
if (isset($flt_product_category) && $flt_product_category != "") {
	$sql_cl 	.= " AND a.id = '" . trim($flt_product_category) . "' ";
}
if (isset($flt_product_category_type) && $flt_product_category_type != "") {
	$sql_cl 	.= " AND a.category_type = '" . trim($flt_product_category_type) . "' ";
}
...
$sql_cl 		.= " ORDER BY a.enabled DESC, a.category_type, a.category_name  "; // echo $sql_cl;
$result_cl 		= $db->query($conn, $sql_cl);
$count_cl 		= $db->counter($result_cl);
$page_heading 	= "List of Product Categories";
?>
<!-- BEGIN: Page Main-->
							...
							<div class="card-content custom_padding_card_content_table_top">
								...
								<form method="post" autocomplete="off" enctype="multipart/form-data"  action="<?php echo "?string=" . encrypt('module_id=' . $module_id . '&page=' . $page); ?>">
									...
									<div class="row">
										...
										<div class="input-field col m2 s12">
											<?php
											$field_name 	= "flt_product_category_type";
											$field_label 	= "Category Type";
											$sql1 			= "SELECT distinct category_type FROM product_categories WHERE enabled = 1  ORDER BY category_type ";
											$result1 		= $db->query($conn, $sql1);
											$count1 		= $db->counter($result1);
											?>
											<i class="material-icons prefix">question_answer</i>
											<div class="select2div">
												<select id="<?= $field_name; ?>" name="<?= $field_name; ?>" class=" select2 browser-default select2-hidden-accessible validate <?php if (isset(${$field_name . "_valid"})) {
													...
													if ($count1 > 0) {
														$row1	= $db->fetch($result1);
														foreach ($row1 as $data2) { ?>
															<option value="<?php echo $data2['category_type']; ?>" <?php if (isset(${$field_name}) && ${$field_name} == $data2['category_type']) { ?> selected="selected" <?php } ?>><?php echo $data2['category_type']; ?></option>
													<?php }
													} ?>
												</select>
												<label for="<?= $field_name; ?>">
								...
								<div class="row">
									<div class="text_align_right">
										<?php 
										$table_columns	= array('SNo', 'Category Name','Category Type','Actions');
										$k 				= 0;
												...
												if ($count_cl > 0) {
													...
													foreach ($row_cl as $data) {
														...
														<tr>
															...
															<td class="col-<?= set_table_headings($table_columns[1]);?>"><?php if ($data['category_name'] != "") echo ucwords(strtolower($data['category_name'])); ?></td>
															<td class="col-<?= set_table_headings($table_columns[2]);?>"><?php if ($data['category_type'] != "") echo ucwords(strtolower($data['category_type'])); ?></td>
