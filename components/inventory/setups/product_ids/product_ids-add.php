...
if ($cmd == 'edit') {
	$title_heading = "Update Product ID";
	$button_val = "Save";
}
if ($cmd == 'add') {
	$title_heading 	= "Add Product ID";
	$button_val 	= "Add";
	$id 			= "";
}
	...
	if (empty($error)) {
		if ($cmd == 'add') {
			if (access("add_perm") == 0) {
				...
				$sql_dup	= " SELECT a.* FROM product_ids a  WHERE a.product_id	= '" . $product_id . "' ";
				$result_dup	= $db->query($conn, $sql_dup);
				$count_dup	= $db->counter($result_dup);
				if ($count_dup == 0) {
					$sql6 = "INSERT INTO " . $selected_db_name . ".product_ids(subscriber_users_id, product_id, add_date, add_by, add_ip)
							VALUES('" . $subscriber_users_id . "', '" . $product_id . "', '" . $add_date . "', '" . $_SESSION['username'] . "', '" . $add_ip . "')";
			...
			if (access("edit_perm") == 0) {
				...
				$sql_dup	= " SELECT a.* FROM product_ids a WHERE a.product_id = '" . $product_id . "' AND a.id != '" . $id . "'";
				$result_dup	= $db->query($conn, $sql_dup);
				$count_dup	= $db->counter($result_dup);
				if ($count_dup == 0) {
