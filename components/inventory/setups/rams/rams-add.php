...
if ($cmd == 'edit') {
	$title_heading = "Update Ram";
	$button_val = "Save";
}
if ($cmd == 'add') {
	$title_heading 	= "Add Ram";
	$button_val 	= "Add";
	$id 			= "";
}
if ($cmd == 'edit' && isset($id)) {

	$sql_ee				= "SELECT a.* FROM rams a WHERE a.id = '" . $id . "' "; // echo $sql_ee;
	$result_ee			= $db->query($conn, $sql_ee);
	$row_ee				= $db->fetch($result_ee);
	$ram_name		= $row_ee[0]['ram_name'];
}
extract($_POST);
...
if (isset($is_Submit) && $is_Submit == 'Y') {
	if (isset($ram_name) && $ram_name == "") {
		$error['ram_name']	= "Required";
	...
	if (empty($error)) {
		if ($cmd == 'add') {
			if (access("add_perm") == 0) {
				...
				$sql_dup	= " SELECT a.* FROM rams a  WHERE a.ram_name	= '" . $ram_name . "' ";
				$result_dup	= $db->query($conn, $sql_dup);
				$count_dup	= $db->counter($result_dup);
				if ($count_dup == 0) {
					$sql6 = "INSERT INTO " . $selected_db_name . ".rams(subscriber_users_id, ram_name, add_date, add_by, add_ip)
							VALUES('" . $subscriber_users_id . "', '" . $ram_name . "', '" . $add_date . "', '" . $_SESSION['username'] . "', '" . $add_ip . "')";
					...
					$sql_c_up = "UPDATE rams 	SET enabled		= '1', 
												update_date		= '" . $add_date . "',
												update_by		= '" . $_SESSION['username'] . "',
												update_ip		= '" . $add_ip . "'
								WHERE ram_name	= '" . $ram_name . "' ";
			...
			if (access("edit_perm") == 0) {
				...
				$sql_dup	= " SELECT a.* FROM rams a WHERE a.ram_name = '" . $ram_name . "' AND a.id != '" . $id . "'";
				$result_dup	= $db->query($conn, $sql_dup);
				$count_dup	= $db->counter($result_dup);
				if ($count_dup == 0) {
					$sql_c_up = "UPDATE rams SET ram_name	= '" . $ram_name . "', 
															 update_date	= '" . $add_date . "',
															 update_by		= '" . $_SESSION['username'] . "',
															 update_ip		= '" . $add_ip . "'
								WHERE id = '" . $id . "' ";
					...
					$ok = $db->query($conn, $sql_c_up);
					if ($ok) {
						$sql_c_up = "UPDATE rams 	SET enabled		= '0', 
													update_date		= '" . $add_date . "',
													update_by		= '" . $_SESSION['username'] . "',
													update_ip		= '" . $add_ip . "'
									WHERE id = '" . $id . "' ";
