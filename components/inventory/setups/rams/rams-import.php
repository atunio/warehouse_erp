...
$supported_column_titles 	= array("ram_name");
$duplication_columns 		= array("ram_name");
$required_columns 			= array("ram_name");
	...
	if (empty($error)) {
		...
		if (isset($all_data) && sizeof($all_data) > 0) {
			...
			foreach ($all_data  as $data1) {
				if (isset($data1['ram_name']) && $data1['ram_name'] != '' && $data1['ram_name'] != NULL && $data1['ram_name'] != 'blank') {
					$ram_name2 	= $data1['ram_name'];
					...
					if (isset($ids_already) && isset($ram_name2) && in_array($ram_name2, $ids_already)) {
						if ($update_column != "") { // ONLY IF one field 
							$sql6 = "UPDATE " . $selected_db_name . "." . $master_table . " SET update_date 			= '" . $add_date . "', 
																								...
																								" . $update_column . "
									WHERE ram_name 	= '" . $ram_name2 . "' "; //echo "<br><br>".$sql6;
