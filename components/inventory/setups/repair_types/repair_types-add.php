...
if ($cmd == 'edit') {
	$title_heading = "Update Repair Type";
	$button_val = "Save";
}
if ($cmd == 'add') {
	$title_heading 	= "Add Repair Type";
	$button_val 	= "Add";
	$id 			= "";
}
if ($cmd == 'edit' && isset($id)) {

	$sql_ee				= "SELECT a.* FROM repair_types a WHERE a.id = '" . $id . "' "; // echo $sql_ee;
	$result_ee			= $db->query($conn, $sql_ee);
	$row_ee				= $db->fetch($result_ee);
	$repair_type_name		= $row_ee[0]['repair_type_name'];
}
extract($_POST);
...
if (isset($is_Submit) && $is_Submit == 'Y') {
	if (isset($repair_type_name) && $repair_type_name == "") {
		$error['repair_type_name']	= "Required";
	...
	if (empty($error)) {
		if ($cmd == 'add') {
			if (access("add_perm") == 0) {
				...
				$sql_dup	= " SELECT a.* FROM repair_types a  WHERE a.repair_type_name	= '" . $repair_type_name . "' ";
				$result_dup	= $db->query($conn, $sql_dup);
				$count_dup	= $db->counter($result_dup);
				if ($count_dup == 0) {
					$sql6 = "INSERT INTO " . $selected_db_name . ".repair_types(subscriber_users_id, repair_type_name, add_date, add_by, add_ip)
					...
					$sql_c_up = "UPDATE repair_types SET enabled	= '1', 
													update_date		= '" . $add_date . "',
													update_by		= '" . $_SESSION['username'] . "',
													update_ip		= '" . $add_ip . "'
								WHERE repair_type_name	= '" . $repair_type_name . "' ";
			...
			if (access("edit_perm") == 0) {
				...
				$sql_dup	= " SELECT a.* FROM repair_types a WHERE a.repair_type_name = '" . $repair_type_name . "' AND a.id != '" . $id . "'";
				$result_dup	= $db->query($conn, $sql_dup);
				$count_dup	= $db->counter($result_dup);
				if ($count_dup == 0) {
					$sql_c_up = "UPDATE repair_types SET repair_type_name	= '" . $repair_type_name . "', 
															 update_date	= '" . $add_date . "',
															 update_by		= '" . $_SESSION['username'] . "',
															 update_ip		= '" . $add_ip . "'
								WHERE id = '" . $id . "' ";
					...
					$ok = $db->query($conn, $sql_c_up);
					if ($ok) {
						$sql_c_up = "UPDATE repair_types SET enabled			= '0', 
															update_date		= '" . $add_date . "',
															update_by		= '" . $_SESSION['username'] . "',
															update_ip		= '" . $add_ip . "'
									WHERE id = '" . $id . "' ";
