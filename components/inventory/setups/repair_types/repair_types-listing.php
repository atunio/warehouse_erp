	...
	if (isset($is_Submit_del) && $is_Submit_del == 'Y') {
		...
		if (empty($error)) {
			...
			foreach ($record_ids as $data_d) {
				if ($is_enabled != "") {
					$sql_dl = "UPDATE repair_types SET enabled = '" . $is_enabled . "' WHERE id = '" . $data_d . "' ";
...
if (isset($cmd) && ($cmd == 'disabled' || $cmd == 'enabled') && access("delete_perm") == 0) {
	...
	if (isset($cmd) && $cmd == 'disabled') {
		$sql_c_upd = "UPDATE repair_types set enabled = 0,
	...
	if (isset($cmd) && $cmd == 'enabled') {
		$sql_c_upd = "UPDATE repair_types set enabled = 1,
											update_date = '" . $add_date . "' ,
											update_by 	= '" . $_SESSION['username'] . "' ,
											update_ip 	= '" . $add_ip . "'
					WHERE id = '" . $id . "' ";
...
$sql_cl 		= "	SELECT  a.* FROM repair_types a WHERE 1=1 ";
if (isset($flt_repair_type) && $flt_repair_type != "") {
	$sql_cl 	.= " AND a.id = '" . trim($flt_repair_type) . "' ";
}
...
$page_heading 	= "List of Repair Types";
?>
<!-- BEGIN: Page Main-->
									...
									<div class="row">
										<div class="input-field col m3 s12">
											...
											<div class="select2div">
												<?php
												$field_name     = "flt_repair_type";
												$field_label    = "Repair Type";

												$sql1           = " SELECT  a.* FROM repair_types a ORDER BY repair_type_name";
								...
								<div class="row">
									...
									<div class="col m10 s12">
										<div class="text_align_right">
											<?php
											$table_columns	= array('SNo', 'Repair Type', 'Actions');
											$k 				= 0;
														...
														foreach ($row_cl as $data) {
															...
															<tr>
																...
																<td class="col-<?= set_table_headings($table_columns[$column_no]); ?>">
																	...
																	<?php echo ucwords(strtolower($data['repair_type_name']));
																	$column_no++; ?>
																</td>
