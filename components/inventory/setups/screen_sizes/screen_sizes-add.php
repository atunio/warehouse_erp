...
if (isset($test_on_local) && $test_on_local == 1 && $cmd == 'add') {
 	$screen_size_name	= "Size ".date('Ymd-His');
}
$db 					= new mySqlDB;
...
if ($cmd == 'edit') {
	$title_heading = "Update Screen Size";
	$button_val = "Save";
}
if ($cmd == 'add') {
	$title_heading 	= "Add Screen Size";
	$button_val 	= "Add";
	$id 			= "";
}
if ($cmd == 'edit' && isset($id)) {

	$sql_ee				= "SELECT a.* FROM screen_sizes a WHERE a.id = '" . $id . "' "; // echo $sql_ee;
	$result_ee			= $db->query($conn, $sql_ee);
	$row_ee				= $db->fetch($result_ee);
	$screen_size_name		= $row_ee[0]['screen_size_name'];
}
extract($_POST);
...
if (isset($is_Submit) && $is_Submit == 'Y') {
	if (isset($screen_size_name) && $screen_size_name == "") {
		$error['screen_size_name']	= "Required";
	...
	if (empty($error)) {
		if ($cmd == 'add') {
			if (access("add_perm") == 0) {
				...
				$sql_dup	= " SELECT a.* FROM screen_sizes a  WHERE a.screen_size_name	= '" . $screen_size_name . "' ";
				$result_dup	= $db->query($conn, $sql_dup);
				$count_dup	= $db->counter($result_dup);
				if ($count_dup == 0) {
					$sql6 = "INSERT INTO " . $selected_db_name . ".screen_sizes(subscriber_users_id, screen_size_name, add_date, add_by, add_ip)
							VALUES('" . $subscriber_users_id . "', '" . $screen_size_name . "', '" . $add_date . "', '" . $_SESSION['username'] . "', '" . $add_ip . "')";
					...
					$sql_c_up = "UPDATE screen_sizes 	SET enabled		= '1', 
														update_date		= '" . $add_date . "',
														update_by		= '" . $_SESSION['username'] . "',
														update_ip		= '" . $add_ip . "'
								WHERE screen_size_name	= '" . $screen_size_name . "' ";
			...
			if (access("edit_perm") == 0) {
				...
				$sql_dup	= " SELECT a.* FROM screen_sizes a WHERE a.screen_size_name = '" . $screen_size_name . "' AND a.id != '" . $id . "'";
				$result_dup	= $db->query($conn, $sql_dup);
				$count_dup	= $db->counter($result_dup);
				if ($count_dup == 0) {
					$sql_c_up = "UPDATE screen_sizes SET screen_size_name	= '" . $screen_size_name . "', 
															 update_date	= '" . $add_date . "',
															 update_by		= '" . $_SESSION['username'] . "',
															 update_ip		= '" . $add_ip . "'
								WHERE id = '" . $id . "' ";
					...
					<form method="post" autocomplete="off">
						...
						<div class="row">
							<div class="input-field col m6 s12">
								<?php
								$field_name 	= "screen_size_name";
								$field_label 	= "Screen Size";
								?>
