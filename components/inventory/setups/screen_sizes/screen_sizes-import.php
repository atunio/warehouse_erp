	...
	if (empty($error)) {
		...
		if (isset($all_data) && sizeof($all_data) > 0) {
			...
			foreach ($all_data  as $data1) {
				if (isset($data1['screen_size_name']) && $data1['screen_size_name'] != '' && $data1['screen_size_name'] != NULL && $data1['screen_size_name'] != 'blank') {
					$screen_size_name2 	= $data1['screen_size_name'];
					...
					if (isset($ids_already) && isset($screen_size_name2) && in_array($screen_size_name2, $ids_already)) {
						if ($update_column != "") { // ONLY IF one field 
							$sql6 = "UPDATE " . $selected_db_name . "." . $master_table . " SET update_date 			= '" . $add_date . "', 
																								...
																								" . $update_column . "
									WHERE screen_size_name 	= '" . $screen_size_name2 . "' "; //echo "<br><br>".$sql6;
