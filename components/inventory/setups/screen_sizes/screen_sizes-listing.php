	...
	if (isset($is_Submit_del) && $is_Submit_del == 'Y') {
		...
		if (empty($error)) {
			...
			foreach ($record_ids as $data_d) {
				if ($is_enabled != "") {
					$sql_dl = "UPDATE screen_sizes SET enabled = '" . $is_enabled . "' WHERE id = '" . $data_d . "' ";
...
if (isset($cmd) && ($cmd == 'disabled' || $cmd == 'enabled') && access("delete_perm") == 0) {
	...
	if (isset($cmd) && $cmd == 'disabled') {
		$sql_c_upd = "UPDATE screen_sizes set enabled = 0,
	...
	if (isset($cmd) && $cmd == 'enabled') {
		$sql_c_upd = "UPDATE screen_sizes set enabled = 1,
											update_date = '" . $add_date . "' ,
											update_by 	= '" . $_SESSION['username'] . "' ,
											update_ip 	= '" . $add_ip . "'
					WHERE id = '" . $id . "' ";
...
$sql_cl 		= "	SELECT  a.* FROM screen_sizes a  WHERE 1=1  ";
if (isset($flt_screen_size) && $flt_screen_size != "") {
	$sql_cl 	.= " AND a.id = '" . trim($flt_screen_size) . "' ";
}
...
$page_heading 	= "List of Screen Sizes";
?>
<!-- BEGIN: Page Main-->
									...
									<div class="row">
										<div class="input-field col m3 s12">
											...
											<div class="select2div">
												<?php
												$field_name     = "flt_screen_size";
												$field_label    = "Screen Size";

												$sql1           = " SELECT  a.* FROM screen_sizes a ORDER BY screen_size_name";
												...
												<select id="<?= $field_name; ?>" name="<?= $field_name; ?>" class=" select2 browser-default select2-hidden-accessible validate <?php if (isset(${$field_name . "_valid"})) {
													...
													if ($count1 > 0) {
														...
														foreach ($row1 as $data2) { ?>
															<option value="<?php echo $data2['id']; ?>" <?php if (isset(${$field_name}) && ${$field_name} == $data2['id']) { ?> selected="selected" <?php } ?>><?php echo $data2['screen_size_name']; ?></option>
													<?php }
													} ?>
												</select>
														...
														foreach ($row_cl as $data) {
															...
															<tr>
																...
																<td class="col-<?= set_table_headings($table_columns[$column_no]); ?>">
																	...
																	<?php echo ucwords(strtolower($data['screen_size_name']));
																	$column_no++; ?>
																</td>
