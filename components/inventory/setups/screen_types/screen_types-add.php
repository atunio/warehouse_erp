...
if (isset($test_on_local) && $test_on_local == 1 && $cmd == 'add') {
 	$screen_type_name	= "SCR ".date('Ymd-His');
}
$db 					= new mySqlDB;
		...
		if ($cmd == 'add') {
			if (access("add_perm") == 0) {
				...
				if ($count_dup == 0) {
					...
					$sql_c_up = "UPDATE screen_types 	SET enabled		= '1', 
															update_date		= '" . $add_date . "',
															update_by		= '" . $_SESSION['username'] . "',
															update_ip		= '" . $add_ip . "'
								WHERE screen_type_name	= '" . $screen_type_name . "' ";
			...
			if (access("edit_perm") == 0) {
				...
				if ($count_dup == 0) {
					...
					$ok = $db->query($conn, $sql_c_up);
					if ($ok) {
						$sql_c_up = "UPDATE screen_types 	SET enabled			= '0', 
																update_date		= '" . $add_date . "',
																update_by		= '" . $_SESSION['username'] . "',
																update_ip		= '" . $add_ip . "'
									WHERE id = '" . $id . "' ";
