	...
	if (isset($is_Submit_del) && $is_Submit_del == 'Y') {
		...
		if (empty($error)) {
			...
			foreach ($record_ids as $data_d) {
				if ($is_enabled != "") {
					$sql_dl = "UPDATE screen_types SET enabled = '" . $is_enabled . "' WHERE id = '" . $data_d . "' ";
...
if (isset($cmd) && ($cmd == 'disabled' || $cmd == 'enabled') && access("delete_perm") == 0) {
	...
	if (isset($cmd) && $cmd == 'disabled') {
		$sql_c_upd = "UPDATE screen_types set enabled = 0,
	...
	if (isset($cmd) && $cmd == 'enabled') {
		$sql_c_upd = "UPDATE screen_types set enabled = 1,
											update_date = '" . $add_date . "' ,
											update_by 	= '" . $_SESSION['username'] . "' ,
											update_ip 	= '" . $add_ip . "'
					WHERE id = '" . $id . "' ";
...
$sql_cl 		= "	SELECT  a.* FROM screen_types a WHERE 1=1  ";
...
$page_heading 	= "List of Screen Types";
?>
<!-- BEGIN: Page Main-->
									...
									<div class="row">
										<div class="input-field col m3 s12">
											...
											<div class="select2div">
												...
												$sql1           = " SELECT  a.* FROM screen_types a ORDER BY screen_type_name";
												...
												<select id="<?= $field_name; ?>" name="<?= $field_name; ?>" class=" select2 browser-default select2-hidden-accessible validate <?php if (isset(${$field_name . "_valid"})) {
													...
													if ($count1 > 0) {
														...
														foreach ($row1 as $data2) { ?>
															<option value="<?php echo $data2['id']; ?>" <?php if (isset(${$field_name}) && ${$field_name} == $data2['id']) { ?> selected="selected" <?php } ?>><?php echo $data2['screen_type_name']; ?></option>
													<?php }
													} ?>
												</select>
								...
								<div class="row">
									...
									<div class="col m10 s12">
										<div class="text_align_right">
											<?php
											$table_columns	= array('SNo', 'Screen Type', 'Actions');
											$k 				= 0;
														...
														foreach ($row_cl as $data) {
															...
															<tr>
																...
																<td class="col-<?= set_table_headings($table_columns[$column_no]); ?>">
																	...
																	<?php echo ucwords(strtolower($data['screen_type_name']));
																	$column_no++; ?>
																</td>
