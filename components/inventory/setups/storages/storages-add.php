...
if ($cmd == 'edit') {
	$title_heading = "Update Storage";
	$button_val = "Save";
}
if ($cmd == 'add') {
	$title_heading 	= "Add Storage";
	$button_val 	= "Add";
	$id 			= "";
}
if ($cmd == 'edit' && isset($id)) {

	$sql_ee				= "SELECT a.* FROM storages a WHERE a.id = '" . $id . "' "; // echo $sql_ee;
	$result_ee			= $db->query($conn, $sql_ee);
	$row_ee				= $db->fetch($result_ee);
	$storage_name		= $row_ee[0]['storage_name'];
}
extract($_POST);
...
if (isset($is_Submit) && $is_Submit == 'Y') {
	if (isset($storage_name) && $storage_name == "") {
		$error['storage_name']	= "Required";
	...
	if (empty($error)) {
		if ($cmd == 'add') {
			if (access("add_perm") == 0) {
				...
				$sql_dup	= " SELECT a.* FROM storages a  WHERE a.storage_name	= '" . $storage_name . "' ";
				$result_dup	= $db->query($conn, $sql_dup);
				$count_dup	= $db->counter($result_dup);
				if ($count_dup == 0) {
					$sql6 = "INSERT INTO " . $selected_db_name . ".storages(subscriber_users_id, storage_name, add_date, add_by, add_ip)
							VALUES('" . $subscriber_users_id . "', '" . $storage_name . "', '" . $add_date . "', '" . $_SESSION['username'] . "', '" . $add_ip . "')";
			...
			if (access("edit_perm") == 0) {
				...
				$sql_dup	= " SELECT a.* FROM storages a WHERE a.storage_name = '" . $storage_name . "' AND a.id != '" . $id . "'";
				$result_dup	= $db->query($conn, $sql_dup);
				$count_dup	= $db->counter($result_dup);
				if ($count_dup == 0) {
					$sql_c_up = "UPDATE storages SET storage_name	= '" . $storage_name . "', 
															 update_date	= '" . $add_date . "',
															 update_by		= '" . $_SESSION['username'] . "',
															 update_ip		= '" . $add_ip . "'
								WHERE id = '" . $id . "' ";
