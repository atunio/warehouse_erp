...
$supported_column_titles 	= array("storage_name");
$duplication_columns 		= array("storage_name");
$required_columns 			= array("storage_name");
	...
	if (empty($error)) {
		...
		if (isset($all_data) && sizeof($all_data) > 0) {
			...
			foreach ($all_data  as $data1) {
				if (isset($data1['storage_name']) && $data1['storage_name'] != '' && $data1['storage_name'] != NULL && $data1['storage_name'] != 'blank') {
					$storage_name2 	= $data1['storage_name'];
					...
					if (isset($ids_already) && isset($storage_name2) && in_array($storage_name2, $ids_already)) {
						if ($update_column != "") { // ONLY IF one field 
							$sql6 = "UPDATE " . $selected_db_name . "." . $master_table . " SET update_date 			= '" . $add_date . "', 
																								...
																								" . $update_column . "
									WHERE storage_name 	= '" . $storage_name2 . "' "; //echo "<br><br>".$sql6;
