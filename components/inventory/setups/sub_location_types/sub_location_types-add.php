...
if (isset($test_on_local) && $test_on_local == 1 && $cmd == 'add') {
	$sub_location_type_name	= "SCR " . date('Ymd-His');
}
$db 					= new mySqlDB;
...
if ($cmd == 'edit') {
	$title_heading = "Update Sub Location Type";
	$button_val = "Save";
}
if ($cmd == 'add') {
	$title_heading 	= "Add Sub Location Type";
	$button_val 	= "Add";
	$id 			= "";
}
...
if (isset($is_Submit) && $is_Submit == 'Y') {
	if (isset($sub_location_type_name) && $sub_location_type_name == "") {
		$error['sub_location_type_name']	= "Required";
	...
	if (empty($error)) {
		if ($cmd == 'add') {
			if (access("add_perm") == 0) {
				...
				$sql_dup	= " SELECT a.* FROM sub_location_types a  WHERE a.sub_location_type_name	= '" . $sub_location_type_name . "' ";
				$result_dup	= $db->query($conn, $sql_dup);
				$count_dup	= $db->counter($result_dup);
				if ($count_dup == 0) {
					$sql6 = "INSERT INTO " . $selected_db_name . ".sub_location_types(subscriber_users_id, sub_location_type_name, add_date, add_by, add_ip)
							VALUES('" . $subscriber_users_id . "', '" . $sub_location_type_name . "', '" . $add_date . "', '" . $_SESSION['username'] . "', '" . $add_ip . "')";
					$ok = $db->query($conn, $sql6);
					if ($ok) {
						if (isset($error['msg'])) unset($error['msg']);
						$msg['msg_success'] = "Record has been added successfully.";
						$sub_location_type_name = "";
					} else {
						$error['msg'] = "There is Error, Please check it again OR contact Support Team.";
					}
				} else {
					$sql_c_up = "UPDATE sub_location_types SET enabled		= '1', 
															 update_date	= '" . $add_date . "',
															 update_by		= '" . $_SESSION['username'] . "',
															 update_ip		= '" . $add_ip . "'
								WHERE sub_location_type_name = '" . $sub_location_type_name . "' ";
			...
			if (access("edit_perm") == 0) {
				...
				$sql_dup	= " SELECT a.* FROM sub_location_types a WHERE a.sub_location_type_name = '" . $sub_location_type_name . "' AND a.id != '" . $id . "'";
				$result_dup	= $db->query($conn, $sql_dup);
				$count_dup	= $db->counter($result_dup);
				if ($count_dup == 0) {
					...
					$ok = $db->query($conn, $sql_c_up);
					if ($ok) {
						$sql_c_up = "UPDATE sub_location_types SET 	enabled 		= '0', 
																	update_date		= '" . $add_date . "',
																	update_by		= '" . $_SESSION['username'] . "',
																	update_ip		= '" . $add_ip . "'
									WHERE id = '" . $id . "' ";
					...
					<form method="post" autocomplete="off">
						...
						<div class="row">
							<div class="input-field col m6 s12">
								<?php
								$field_name 	= "sub_location_type_name";
								$field_label 	= "Type";
								?>
