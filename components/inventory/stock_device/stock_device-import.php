							...
							if ($key == 'p_inventory_status') {
								...
								if ($data != '' && $data != NULL && $data != '-' && $data != 'blank') {
									...
									if ($count1 > 0) {
										...
										if ($ok) {
											$id_for_insert = mysqli_insert_id($conn);
											$columns 		.= ", " . $key;
											$column_data 	.= ", '" . $id_for_insert . "'";
										}
									}
								}
								...
								if ($data != '' && $data != NULL && $data != '-' && $data != 'blank') {
									...
									if ($count1 > 0) {
										...
										$sql6 = "INSERT INTO " . $selected_db_name . "." . $table . "(subscriber_users_id, warehouse_id, " . $field . ", purpose, add_date, add_by, add_ip, add_timezone)
												VALUES('" . $subscriber_users_id . "', '1', '" . $data . "', 'Stock', '" . $add_date . "', '" . $_SESSION['username'] . "', '" . $add_ip . "', '" . $timezone . "')";
										$ok = $db->query($conn, $sql6);
										if ($ok) {
											$id_for_insert	= mysqli_insert_id($conn);
											$columns 		.= ", " . $key;
											...
											foreach ($data as $row_c1) {
												...
												foreach ($row_c1 as $cell_c1) {
													...
													if ($column_name_c1 == 'status') {
														$column_name_c1 = "inventory_status";
													}

													if ($column_name_c1 == 'total_stock') {
														$column_name_c1 = "p_total_stock";
													}
