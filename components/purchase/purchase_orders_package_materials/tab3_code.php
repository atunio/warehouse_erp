	...
	if (empty($error3)) {
		if (po_permisions("Pkg_Receive") == 0) {
			...
			foreach ($receviedProductIds as $receviedProductId) {
				$sql5_pks1	= "	SELECT b.package_id, c.logistics_cost, b.order_price, b.po_id, a.logistic_id
...
if (isset($_POST['is_Submit_tab3']) && $_POST['is_Submit_tab3'] == 'Y') {
	...
	if (!isset($receiving_location) || (isset($receiving_location)  && sizeof($receiving_location) == "0")) {
		$error3['receiving_location'] = "Required";
	} else {
	...
	if (!isset($receiving_qties) || (isset($receiving_qties)  && sizeof($receiving_qties) == "0")) {
		$error3['receiving_qties'] = "Required";
	} else {
					...
					if ($receiving_qty > 0) {
						for ($m = 0; $m < $receiving_qty; $m++) {
							...
							$sql6 = "INSERT INTO package_materials_order_detail_receive(logistic_id, po_detail_id, price, add_by_user_id, sub_location_id, duplication_check_token, add_date,  add_by, add_ip, add_timezone, added_from_module_id)
									VALUES('" . $logistic_id . "', '" . $key . "',   '" . $order_price . "', '" . $_SESSION['user_id'] . "', '" . $receiving_location_add . "', '" . $duplication_check_token . "', '" . $add_date . "', '" . $_SESSION['username'] . "', '" . $add_ip . "', '" . $timezone . "', '" . $module_id . "')";
