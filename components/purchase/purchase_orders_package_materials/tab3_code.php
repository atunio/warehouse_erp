...
if (isset($logistic_id) && $logistic_id > 0) {
	...
	if ($counter_ee1 > 0) {
		...
		$logistic_id_for_receive	= $row_ee1111[0]['id'];
	}
}
	...
	if (empty($error3)) {
		if (po_permisions("Pkg_Receive") == 0) {
			...
			foreach ($receviedProductIds as $receviedProductId_1) {
				...
				$sql5_pks1	= "	SELECT b.package_id, c.logistics_cost, b.order_price, b.po_id, D.box_qty
								FROM package_materials_order_detail b 
								INNER JOIN package_materials_orders c ON c.id = b.po_id
								INNER JOIN package_materials_order_detail_logistics d ON d.po_detail_id = b.id
								WHERE d.id = '" . $receviedProductId . "' ";
				...
				if ($count5_pks1 > 0) {
					...
					$box_qty1			= $row_pd5_pks1[0]['box_qty'];
					...
					$sql_c_up 	= "UPDATE packages SET 
														stock_in_hand			= (stock_in_hand-" . $box_qty1 . "),
					...
					$sql_c_up = "DELETE FROM  package_materials_order_detail_receive WHERE logistic_id 	= '" . $receviedProductId . "' ";
					...
					if ($ok) {
						...
						if ($count5_pks1 == 0) {
							...
							$sql_c_up = "UPDATE  package_materials_order_detail_logistics SET 	 
																								logistics_status		= '10',
	...
	if (empty($error3)) {
		if (po_permisions("Pkg_Receive") == 0) {
			...
			if ($counter_ee1 == 0) {
				...
				$sql_pd3	= "	SELECT a.*, b.id as logistic_id, b.box_qty
								FROM package_materials_order_detail a  
								INNER JOIN package_materials_order_detail_logistics b ON b.po_detail_id = a.id
								WHERE 1 	= 1 
								AND b.id 	= '" . $logistic_id_for_receive . "' ";
				$result_pd3	= $db->query($conn, $sql_pd3);
				$count_pd3	= $db->counter($result_pd3);
				if ($count_pd3 > 0) {
					$row_pd3			= $db->fetch($result_pd3);
					$box_qty			= $row_pd3[0]['box_qty'];
					$inv_product_id		= $row_pd3[0]['package_id'];
					$order_price		= $row_pd3[0]['order_price'];
					$logistic_id		= $row_pd3[0]['logistic_id'];
					$po_detail_id		= $row_pd3[0]['id'];
					...
					for ($m = 0; $m < $box_qty; $m++) {
						...
						$sql6 = "INSERT INTO package_materials_order_detail_receive(logistic_id, po_detail_id, price, add_by_user_id, sub_location_id, duplication_check_token, add_date,  add_by, add_ip, add_timezone, added_from_module_id)
								 VALUES('" . $logistic_id . "', '" . $po_detail_id . "',   '" . $order_price . "', '" . $_SESSION['user_id'] . "', '" . $receiving_location . "', '" . $duplication_check_token . "', '" . $add_date . "', '" . $_SESSION['username'] . "', '" . $add_ip . "', '" . $timezone . "', '" . $module_id . "')";
