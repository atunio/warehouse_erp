	...
	if (isset($is_Submit_del) && $is_Submit_del == 'Y') {
		...
		if (empty($error)) {
			...
			foreach ($record_ids as $data_d) {
				if ($is_enabled != "") {
					$sql_dl = "UPDATE purchasing_agents SET enabled = '" . $is_enabled . "' WHERE id = '" . $data_d . "' ";
...
if (isset($cmd) && ($cmd == 'disabled' || $cmd == 'enabled') && access("delete_perm") == 0) {
	...
	if (isset($cmd) && $cmd == 'disabled') {
		$sql_c_upd = "UPDATE purchasing_agents set enabled = 0,
		...
		if ($enabe_ok) {
			$msg['msg_success'] = "Purchaseing Agent has been deleted.";
		} else {
			$error['msg'] = "There is Error, record does not update, Please check it again OR contact Support Team.";
		}
	}
	if (isset($cmd) && $cmd == 'enabled') {
		$sql_c_upd = "UPDATE purchasing_agents set 	enabled 	= 1,
											update_date = '" . $add_date . "' ,
											update_by 	= '" . $_SESSION['username'] . "' ,
											update_ip 	= '" . $add_ip . "'
					WHERE id = '" . $id . "' ";
		$enabe_ok = $db->query($conn, $sql_c_upd);
		if ($enabe_ok) {
			$msg['msg_success'] = "Purchaseing Agent has been enabled.";
		}
	}
}

$sql_cl 		= "	SELECT a.* 
					FROM purchasing_agents a 
 					WHERE 1=1  ";
if (isset($flt_agent) && $flt_agent != "") {
	$sql_cl 	.= " AND a.id = '" . trim($flt_agent) . "' ";
}
...
$page_heading 	= "List of Purchaseing Agents";
?>
<!-- BEGIN: Page Main-->
								...
								<form method="post" autocomplete="off" enctype="multipart/form-data" action="<?php echo "?string=" . encrypt('module_id=' . $module_id . '&page=' . $page); ?>">
									...
									<div class="row">
										<div class="input-field col m4 s12">
											<?php
											$field_name 	= "flt_agent";
											$field_label 	= "Purchaseing Agent";
											$sql1 			= "SELECT a.* FROM purchasing_agents a  ORDER BY agent_name ";
												...
												<select id="<?= $field_name; ?>" name="<?= $field_name; ?>" class=" select2 browser-default select2-hidden-accessible validate <?php if (isset(${$field_name . "_valid"})) {
													...
													if ($count1 > 0) {
														...
														foreach ($row1 as $data2) { ?>
															<option value="<?php echo $data2['id']; ?>" <?php if (isset(${$field_name}) && ${$field_name} == $data2['id']) { ?> selected="selected" <?php } ?>><?php echo $data2['agent_name']; ?></option>
													<?php }
													} ?>
												</select>
								...
								<div class="row">
									...
									<div class="col m10 s12">
										<div class="text_align_right">
											<?php
											$table_columns	= array('SNo', 'Agent ID', 'Agent Name', 'Phone', 'Address', 'Note About Agent', 'Actions');
											$k 				= 0;
												...
												foreach ($row_cl as $data) {
													...
													<tr>
														...
														<td class="col-<?= set_table_headings($table_columns[$column_no]); ?>">
															...
															<?php echo $data['agent_no']; ?>
														</td>
														<td class="col-<?= set_table_headings($table_columns[$column_no]); ?>"><?php if ($data['agent_name'] != "") echo ucwords(strtolower($data['agent_name'])); ?></td>
														<td class="col-<?= set_table_headings($table_columns[$column_no]); ?>"><?php echo $data['phone_no']; ?></td>
														<td class="col-<?= set_table_headings($table_columns[$column_no]); ?>">
															<?php
															if ($data['address'] != '') {
														...
														<td class="col-<?= set_table_headings($table_columns[$column_no]); ?>"><?php echo $data['note_about_agent']; ?></td>
