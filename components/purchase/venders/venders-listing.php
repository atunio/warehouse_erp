	...
	if (isset($is_Submit_del) && $is_Submit_del == 'Y') {
		...
		if (empty($error)) {
			...
			foreach ($record_ids as $data_d) {
				if ($is_enabled != "") {
					$sql_dl = "UPDATE venders SET enabled = '" . $is_enabled . "' WHERE id = '" . $data_d . "' ";
								...
								<form method="post" autocomplete="off" enctype="multipart/form-data" action="<?php echo "?string=" . encrypt('module_id=' . $module_id . '&page=' . $page); ?>">
									...
									<div class="row">
										<div class="input-field col m2 s12">
											<?php
											$field_name 	= "flt_vendor";
											$field_label 	= "Vendor";
											$sql1 			= "SELECT a.* FROM venders a  ORDER BY vender_name ";
												...
												foreach ($row_cl as $data) {
													...
													<tr>
														...
														<td class="col-<?= set_table_headings($table_columns[$column_no]); ?>">
															...
															<?php echo $data['vender_no'];
															$column_no++; ?>
														</td>
														<td class="col-<?= set_table_headings($table_columns[$column_no]); ?>">
															<?php
															if ($data['vender_name'] != "") {
																echo ucwords(strtolower($data['vender_name']));
															}
															$column_no++; ?>
														</td>
														<td class="col-<?= set_table_headings($table_columns[$column_no]); ?>">
														...
														<td class="col-<?= set_table_headings($table_columns[$column_no]); ?>">
															<?php
															echo number_format($data['credit_balance'], 2);
															$column_no++; ?>
														</td>
