...
if ($cmd == 'edit') {
	$title_heading = "Update store";
	$button_val = "Save";
}
if ($cmd == 'add') {
	$title_heading 	= "Add store";
	$button_val 	= "Add";
	$id 			= "";
}
if ($cmd == 'edit' && isset($id)) {

	$sql_ee				= "SELECT a.* FROM stores a WHERE a.id = '" . $id . "' "; // echo $sql_ee;
	...
	if (empty($error)) {
		...
		if ($cmd == 'add') {
			if (access("add_perm") == 0) {
				...
				$sql_dup	= " SELECT a.* FROM stores a  WHERE a.store_name	= '" . $store_name . "' ";
				$result_dup	= $db->query($conn, $sql_dup);
				$count_dup	= $db->counter($result_dup);
				if ($count_dup == 0) {
					$sql6 = "INSERT INTO " . $selected_db_name . ".stores(subscriber_users_id, store_name, add_date, add_by, add_ip)
			...
			if (access("edit_perm") == 0) {
				...
				$sql_dup	= " SELECT a.* FROM stores a WHERE a.store_name = '" . $store_name . "' AND a.id != '" . $id . "'";
				$result_dup	= $db->query($conn, $sql_dup);
				$count_dup	= $db->counter($result_dup);
				if ($count_dup == 0) {
					$sql_c_up = "UPDATE stores SET store_name	= '" . $store_name . "', 
