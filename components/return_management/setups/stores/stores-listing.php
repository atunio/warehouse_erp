...
if (isset($cmd) && ($cmd == 'disabled' || $cmd == 'enabled') && access("delete_perm") == 0) {
	$error['msg'] = "You do not have edit stores.";
} 
else {
	if (isset($cmd) && $cmd == 'disabled') {
		$sql_c_upd = "UPDATE stores set enabled = 0,
	...
	if (isset($cmd) && $cmd == 'enabled') {
		$sql_c_upd = "UPDATE stores set enabled = 1,
											update_date = '" . $add_date . "' ,
											update_by 	= '" . $_SESSION['username'] . "' ,
											update_ip 	= '" . $add_ip . "'
					WHERE id = '" . $id . "' ";
...
$sql_cl 		= "	SELECT  a.* from stores  a ORDER BY a.enabled DESC  "; // echo $sql_cl;
$result_cl 		= $db->query($conn, $sql_cl);
$count_cl 		= $db->counter($result_cl);
$page_heading 	= "List of stores";
?>
<!-- BEGIN: Page Main-->
								...
								<div class="card-content custom_padding_card_content_table_top">
									...
									<div class="row">
										<div class="text_align_right">
											<?php 
											$table_columns	= array('SNo', 'Store Name','Actions');
											$k 				= 0;
													...
													if ($count_cl > 0) {
														...
														foreach ($row_cl as $data) {
															...
															<tr>
																...
																<td class="col-<?= set_table_headings($table_columns[1]);?>"><?php echo ucwords(strtolower($data['store_name'])); ?></td>
