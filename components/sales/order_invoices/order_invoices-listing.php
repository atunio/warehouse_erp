...
if (isset($cmd) && ($cmd == 'disabled' || $cmd == 'enabled') && access("delete_perm") == 0) {
	...
	if (isset($cmd) && $cmd == 'disabled') {
		$sql_c_upd = "UPDATE sale_order_invoices set enabled = 0,
		...
		if ($enabe_ok) {
			$msg['msg_success'] = "Invoice has been disabled.";
		} else {
			$error['msg'] = "There is Error, record does not update, Please check it again OR contact Support Team.";
		}
	}
	if (isset($cmd) && $cmd == 'enabled') {
		$sql_c_upd = "UPDATE sale_order_invoices set 	enabled 	= 1,
											update_date = '" . $add_date . "' ,
											update_by 	= '" . $_SESSION['username'] . "' ,
											update_ip 	= '" . $add_ip . "'
					WHERE id = '" . $id . "' ";
		$enabe_ok = $db->query($conn, $sql_c_upd);
		if ($enabe_ok) {
			$msg['msg_success'] = "Invoice has been reactive.";
		}
	}
}
								...
								<div class="card-content custom_padding_card_content_table_top">
									...
									<div class="row">
										<div class="text_align_right">
											<?php 
											$table_columns	= array('SNo', 'Invoice No', 'Invoice Date','Actions');
											$k 				= 0;
													...
													if ($count_cl > 0) {
														...
														foreach ($row_cl as $data) {
															...
															<tr>
																...
																<td class="col-<?= set_table_headings($table_columns[1]);?>"><?php echo $data['invoice_no']; ?></td>
																<td class="col-<?= set_table_headings($table_columns[2]);?>"><?php echo ucwords(strtolower($data['invoice_date'])); ?></td>
																...
																<td class="text-align-center col-<?= set_table_headings($table_columns[3]);?>">
																	...
																	<?php } else if ($data['enabled'] == 1 && $data['is_posted'] == 0 && access("delete_perm") == 1) { ?>
																	...
																	<?php } 
																	if ($data['enabled'] == 1 && access("print_perm") == 1) { ?>
