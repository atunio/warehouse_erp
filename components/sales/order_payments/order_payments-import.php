...
$supported_column_titles	= array(
	...
	"email_secondary",
	"address_primary",
	"address_primary_city",
	"address_primary_state",
	"address_primary_country",
	"address_secondary",
	"address_secondary_city",
	"address_secondary_state",
	"address_secondary_country",
	"fax_no",
	"note_about_customer"
);
$duplication_columns 		= array("phone_primary");
$required_columns 			= array("customer_name", "phone_primary");
				...
				if (!in_array($data1[$dup_data], $duplicate_data_array)) {
					...
					foreach ($data1 as $key => $data) {
						...
						if ($key != "" && $key != 'is_insert') {
							...
							if ($key == 'address_primary_country' || $key == 'address_secondary_country') {
								$table = "countries";
								if ($data != '' && $data != NULL && $data != '-' && $data != 'blank') {
									$sql1 			= "SELECT * FROM " . $table . " WHERE country_name = '" . $data . "' ";
									$result1 		= $db->query($conn, $sql1);
									$count1 		= $db->counter($result1);
									if ($count1 > 0) {
										...
										$sql6 = "INSERT INTO " . $selected_db_name . "." . $table . "(country_name, add_date, add_by, add_by_user_id, add_ip, add_timezone, added_from_module_id)
												 VALUES('" . $data . "', '" . $add_date . "', '" . $_SESSION['username'] . "', '" . $_SESSION['user_id'] . "', '" . $add_ip . "', '" . $timezone . "', '" . $module_id . "')";
										$ok = $db->query($conn, $sql6);
										if ($ok) {
											$inserted_id = mysqli_insert_id($conn);
											$columns 		.= ", " . $key;
											$column_data 	.= ", '" . $inserted_id . "'";
										}
									}
								}
							} else {
					...
					if ($ok) {
						...
						$customer_no	= "C" . $id;
						$sql6		= "UPDATE " . $master_table . " SET customer_no = '" . $customer_no . "' WHERE id = '" . $id . "' ";
						$db->query($conn, $sql6);
						$added++;
					}
				}
