...
if (isset($_SESSION["username"]) && isset($_SESSION["user_id"]) && isset($_SESSION["schoolDirectory"]) && $_SESSION["schoolDirectory"] == $project_folder &&  isset($_SESSION["project_name"]) && $_SESSION["project_name"] == $project_name) {
	...
	if ($check_module_permission == "") {
		...
		$sql_cl = "	SELECT 	b.so_no, b.customer_po_no, DATE_FORMAT(b.order_date, '%M %d %Y') as order_date, CONCAT(h.sub_location_name, ' ', IFNULL(h.sub_location_type, '')) AS location, 
							d.category_name, c1.product_uniqueid as product_id, c1.product_desc, c.serial_no 
					...
					ORDER BY h.sub_location_name, d.category_name, c1.product_uniqueid, c.serial_no "; //AND a2.is_final_pricing = 1
		// echo $sql_cl;die;
		...
		if ($count_cl > 0) {
			...
			// Submission from
			$filename = "sales_order_details_" . date('YmdHis') . ".csv";
