...
if (isset($_SESSION["username"]) && isset($_SESSION["user_id"]) && isset($_SESSION["schoolDirectory"]) && $_SESSION["schoolDirectory"] == $project_folder &&  isset($_SESSION["project_name"]) && $_SESSION["project_name"] == $project_name) {
	...
	if ($check_module_permission == "") {
		...
		$sql_cl = "	SELECT b.so_no, b.customer_invoice_no, DATE_FORMAT(b.order_date, '%M %d %Y') as order_date, d.category_name, COUNT(a.id) AS total_qty
					...
					ORDER BY c1.product_category  "; //AND a2.is_final_pricing = 1
		// echo $sql_cl;die;
		...
		if ($count_cl > 0) {
			...
			// Submission from
			$filename = "sales_order_summary_" . date('YmdHis') . ".csv";
