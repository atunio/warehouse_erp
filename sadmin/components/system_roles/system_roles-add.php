...
$db = new mySqlDB;
if ($cmd == 'edit') {
	$title_heading = "Edit System User Role";
	$button_val = "Edit";
}
if ($cmd == 'add') {
	$title_heading 	= "Add System User Role";
	$button_val 	= "Add";
}
extract($_POST);
...
if (isset($is_Submit) && $is_Submit == 'Y') {
	...
	if (empty($error)) {
		if ($cmd == 'add') {
			$sql1 		= " SELECT * FROM roles WHERE enabled = 1  AND role_name = '" . $role_name . "'";
			$result1 	= $db->query($conn, $sql1);
			$count1 	= $db->counter($result1);
			if ($count1 == 0) {
				$sql_c_u = "INSERT INTO roles (role_name, add_date, add_by, add_ip)
			...
			$count1 	= $db->counter($result1);
			if ($count1 == 0) {
				$sql_c_up = "UPDATE roles SET role_name = '" . $role_name . "',
