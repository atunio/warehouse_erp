...
$db = new mySqlDB;
$title_heading 	= "Set System User Permissions";
$button_val 	= "Submit";
extract($_POST);
...
if (isset($is_Submit) && $is_Submit == 'Y') {
	if (empty($error)) {
		...
		$sql_del = "DELETE FROM  role_permissions  WHERE role_id 	= '" . $id . "' ";
		$db->query($conn, $sql_del);
		foreach ($all_id as $key => $value) {
			$sql_c1 	= " SELECT * FROM role_permissions	WHERE role_id = " . $id . " AND menu_id = " . $key . " ";
			$rs_c1 		= $db->query($conn, $sql_c1);
			$count_c1 	= $db->counter($rs_c1);
			if ($count_c1 == 0) {
				$sql_c_u = "INSERT INTO role_permissions (role_id, menu_id, add_date, add_by, add_ip)
	...
	$sql_ee 	= " SELECT * FROM roles WHERE enabled = 1 AND id = '" . $id . "' ";
				...
				<div class="card-content">
					...
					<form method="post" autocomplete="off">
						...
						<div class="row">
							...
							<?php
							$sql2		= " SELECT * FROM menus WHERE m_level = 1 AND enabled = 1 ORDER BY sort_order ";
							$result_2 	= $db->query($conn, $sql2);
							$row_2 		= $db->fetch($result_2);
							foreach ($row_2 as $data) {
								$parent_id_level_1 = $data['id'];
								$sql_level1 	= " SELECT * FROM role_permissions 
								...
								if ($count_3 > 0) {
									...
									foreach ($row_3 as $data2) {
										...
										$sql_level2 		= " SELECT * FROM role_permissions
										...
										if ($count4 > 0) {
											...
											foreach ($row4 as $data3) {
												...
												$sql_level3 		= " SELECT * FROM role_permissions
												...
												if ($count5 > 0) {
													...
													foreach ($row5 as $data4) {
														...
														$sql_level4 		= " SELECT * FROM role_permissions
